...
def parse_option():
    ...
    parser.add_argument('--cfg', type=str, metavar="FILE", default='configs/RESACEUNET.yaml',help='path to config file', )
    parser.add_argument(
        "--opts",
...
def main(config=config,fine_tune=None,using_pretrain=None):
    ...
    if config.TRAIN.LOSS.NAME == 'bce_dice_loss':
        ...
        criterion = bce_dice_loss(bce_weight=config.TRAIN.LOSS.WEIGHT, smooth_nr=0., smooth_dr=1e-6,smoothing=config.TRAIN.LOSS.LABELSMOOTH)
