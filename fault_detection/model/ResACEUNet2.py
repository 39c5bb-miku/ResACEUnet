...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ace_block = ACE(
            ...
            proj_size=proj_size,
            num_heads=num_heads,
            channel_attn_drop=drop_rate,
            spatial_attn_drop=drop_rate,
            proj_drop=drop_rate,
        )
        ...
        self.conv8 = nn.Sequential(
            nn.Dropout3d(0.2, False), nn.Conv3d(hidden_size, hidden_size, 1)
        )
    ...
    def forward(self, x):
        ...
        if self.pos_embed is not None:
            x = x + self.pos_embed
        attn = x + self.gamma * self.ace_block(
            self.norm(x).permute(0, 2, 1).reshape(B, C, H, W, D)
        )
...
class ACE(nn.Module):
    ...
    def __init__(
        ...
        self.dep_conv = nn.Conv3d(
            16 * hidden_size // self.num_heads,
            hidden_size,
            kernel_size=3,
            bias=True,
            groups=hidden_size // self.num_heads,
            padding=1,
        )
    ...
    def forward(self, x):
        ...
        # B, 4*nhead, H*W*D, C//nhead
        f_all = qkvv.reshape(B, H * W * D, 4 * self.num_heads, -1).permute(0, 2, 1, 3)
        f_conv = (
            self.fc(f_all)
            .permute(0, 3, 1, 2)
            .reshape(B, 16 * x.shape[1] // self.num_heads, H, W, D)
        )  # B, 16*C//nhead, H, W, D
        # group conovlution
...
if __name__ == "__main__":
    ...
    inchannel = 1
    outchannel = 1
    model = ResACEUNet2(in_channels=inchannel, out_channels=outchannel, img_size=64)
