...
class ResACEUnet2(nn.Module):
    ...
    def __init__(
        ...
        self.ace_encoder = ACEEncoder(
            input_size=[(img_size[0]//4)**3,(img_size[0]//8)**3,(img_size[0]//16)**3,(img_size[0]//32)**3],
    ...
    def forward(self, x_in):
        ...
        if self.do_ds:
            out = [self.outact(self.out1(out)), self.outact(self.out2(dec1)), self.outact(self.out3(dec2))]
        else:
            out = self.outact(self.out1(out))
...
class ACEEncoder(nn.Module):
    ...
    def __init__(self, input_size, dims, proj_size, depths, num_heads, spatial_dims, in_channels, dropout, transformer_dropout_rate):
        super().__init__()
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(TransformerBlock(input_size=input_size[i], hidden_size=dims[i],  proj_size=proj_size[i], num_heads=num_heads,dropout_rate=transformer_dropout_rate, pos_embed=True))
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ace_block = ACE(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,spatial_attn_drop=dropout_rate,proj_drop=dropout_rate)
